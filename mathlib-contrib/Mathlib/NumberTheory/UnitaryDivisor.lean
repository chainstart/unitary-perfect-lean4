...
is multiplicative: `σ*(mn) = σ*(m) · σ*(n)` when `gcd(m,n) = 1`.

## Main Definitions

* `Nat.IsUnitaryDivisor d n`: Predicate stating that `d` is a unitary divisor of `n`.
...
theorem isUnitaryDivisor_one (n : ℕ) (hn : n ≠ 0) : IsUnitaryDivisor 1 n := by
  refine ⟨one_dvd n, hn, ?_⟩
  simp [Nat.div_one]

/-- Every nonzero `n` is a unitary divisor of itself. -/
theorem isUnitaryDivisor_self (n : ℕ) (hn : n ≠ 0) : IsUnitaryDivisor n n := by
...
theorem mem_unitaryDivisors {d n : ℕ} (hn : n ≠ 0) :
  ...
  simp only [unitaryDivisors, IsUnitaryDivisor, Finset.mem_filter, Nat.mem_divisors]
  constructor
  · intro ⟨⟨hd, _⟩, hgcd⟩
    exact ⟨hd, hn, hgcd⟩
  · intro ⟨hd, _, hgcd⟩
...
private lemma unitary_divisor_eq_gcd_mul_gcd {m n d : ℕ} (hcoprime : Nat.Coprime m n)
    (h : IsUnitaryDivisor d (m * n)) : d = Nat.gcd d m * Nat.gcd d n :=
...
theorem isUnitaryDivisor_mul_of_coprime {m n d₁ d₂ : ℕ} (hcoprime : Nat.Coprime m n)
    (hm : m ≠ 0) (hn : n ≠ 0) (h1 : IsUnitaryDivisor d₁ m) (h2 : IsUnitaryDivisor d₂ n) :
    IsUnitaryDivisor (d₁ * d₂) (m * n) := by
...
theorem unitaryDivisorSum_mul {m n : ℕ} (hmn : Nat.gcd m n = 1) (hm : m ≠ 0) (hn : n ≠ 0) :
    σ* (m * n) = σ* m * σ* n := by
  unfold unitaryDivisorSum
  have hcoprime : Nat.Coprime m n := hmn
  ...
  have h_sum_bij : ∑ d ∈ unitaryDivisors (m * n), d =
    ...
    · intro d hd
      ...
      have ⟨h1, h2⟩ := ⟨gcd_isUnitaryDivisor_left hcoprime hm hn h_in,
                        gcd_isUnitaryDivisor_right hcoprime hm hn h_in⟩
      exact ⟨(mem_unitaryDivisors hm).mpr h1, (mem_unitaryDivisors hn).mpr h2⟩
    · intro p hp
      simp only [Finset.mem_product] at hp
      have h1 : IsUnitaryDivisor p.1 m := (mem_unitaryDivisors hm).mp hp.1
      have h2 : IsUnitaryDivisor p.2 n := (mem_unitaryDivisors hn).mp hp.2
      exact (mem_unitaryDivisors (mul_ne_zero hm hn)).mpr
        (isUnitaryDivisor_mul_of_coprime hcoprime hm hn h1 h2)
    · intro d hd
      have h_in : IsUnitaryDivisor d (m * n) := (mem_unitaryDivisors (mul_ne_zero hm hn)).mp hd
  ...
  | succ k ih =>
    ...
    · have hgcd : Nat.gcd d p = 1 := by
      ...
      have hgcd_pow : Nat.gcd d (p ^ (k + 1)) = 1 := by
        ...
        exact Nat.Coprime.pow_left (k + 1) hgcd
      have hd_eq_1 : d = 1 := by
...
theorem unitaryDivisors_prime_pow {p k : ℕ} (hp : Nat.Prime p) (hk : 0 < k) :
  ...
  · intro hd
    ...
    · exact (mem_unitaryDivisors (pow_ne_zero k hp.ne_zero)).mpr
        (isUnitaryDivisor_one _ (pow_ne_zero k hp.ne_zero))
    · exact (mem_unitaryDivisors (pow_ne_zero k hp.ne_zero)).mpr
        (isUnitaryDivisor_self _ (pow_ne_zero k hp.ne_zero))
...
/-- `σ*(n) ≥ n` for nonzero `n`, since `n` is always a unitary divisor of itself. -/
theorem unitaryDivisorSum_ge_self {n : ℕ} (hn : n ≠ 0) : σ* n ≥ n := by
  unfold unitaryDivisorSum
  have h_mem : n ∈ unitaryDivisors n := self_mem_unitaryDivisors hn
  exact Finset.single_le_sum (fun _ _ => Nat.zero_le _) h_mem

/-- `σ*(n) = n` if and only if `n = 1`. -/
theorem unitaryDivisorSum_eq_self_iff {n : ℕ} (hn : n ≠ 0) : σ* n = n ↔ n = 1 := by
  constructor
  · intro h
    by_contra hn1
    push_neg at hn1
    have h1 : n > 1 := Nat.lt_of_le_of_ne (Nat.one_le_iff_ne_zero.mpr hn) (Ne.symm hn1)
    have h_mem_1 : 1 ∈ unitaryDivisors n := one_mem_unitaryDivisors hn
    have h_mem_n : n ∈ unitaryDivisors n := self_mem_unitaryDivisors hn
    have h_ne : (1 : ℕ) ≠ n := Nat.ne_of_lt h1
    have hsum : σ* n ≥ 1 + n := by
      unfold unitaryDivisorSum
      have h_pair : ({1, n} : Finset ℕ) ⊆ unitaryDivisors n := by
