...
-/
import UnitaryPerfect.UnitaryDivisor
...
* `Nat.IsUnitaryPerfect n`: Predicate stating that `n` is a unitary perfect number.

## Main Results

* `Nat.no_odd_unitary_perfect`: There are no odd unitary perfect numbers greater than 1.
* `Nat.IsUnitaryPerfect.even`: Every unitary perfect number is even.
* `Nat.IsUnitaryPerfect.eq_two_pow_mul_odd`: Every unitary perfect number has the form `2^a * k`
...
/-- A single odd prime power cannot be unitary perfect. -/
theorem not_isUnitaryPerfect_odd_prime_pow {p a : ℕ} (hp : Nat.Prime p) (hodd : Odd p)
    (ha : a ≥ 1) : ¬IsUnitaryPerfect (p ^ a) := by
  intro ⟨_, hup⟩
  have ha_ne : a ≠ 0 := Nat.one_le_iff_ne_zero.mp ha
  have hsigma := unitaryDivisorSum_prime_pow hp ha_ne
  rw [hsigma] at hup
...
theorem unitaryDivisorSum_odd_even {n : ℕ} (hn : 1 < n) (hodd : Odd n) : Even (σ* n) := by
  ...
  | coprime a b ha hb hcoprime iha ihb =>
    ...
    have ha_odd : Odd a := by
      rcases Nat.even_or_odd a with heven | hodd_a
      · have : Even (a * b) := Even.mul_right heven b
        exact absurd this (Nat.not_even_iff_odd.symm.mp hodd)
      · exact hodd_a
    have hb_odd : Odd b := by
      rcases Nat.even_or_odd b with heven | hodd_b
      · have : Even (a * b) := Even.mul_left heven a
        exact absurd this (Nat.not_even_iff_odd.symm.mp hodd)
      · exact hodd_b
    have hsa_even : Even (σ* a) := iha ha ha_odd
    exact Even.mul_right hsa_even (σ* b)
...
theorem no_odd_unitary_perfect {n : ℕ} (hodd : Odd n) (hgt1 : n > 1) : ¬IsUnitaryPerfect n := by
  ...
  | prime_pow p k hp_prime =>
    ...
    have hp_ge_3 : p ≥ 3 := by
      ...
      have hp_ne_2 : p ≠ 2 := fun h => by subst h; exact Nat.not_even_iff_odd.symm.mp hp_odd even_two
      omega
...
theorem IsUnitaryPerfect.eq_two_pow_mul_odd {n : ℕ} (h : IsUnitaryPerfect n) :
