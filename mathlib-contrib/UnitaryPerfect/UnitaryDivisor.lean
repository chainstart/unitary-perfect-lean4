...
theorem unitaryDivisorSum_one : σ* 1 = 1 := by
  ...
  simp only [Nat.divisors_one, Finset.filter_singleton, Nat.gcd_self, ite_true, Finset.sum_singleton]
...
theorem unitaryDivisorSum_ge_self {n : ℕ} (hn : n ≠ 0) : σ* n ≥ n := by
  ...
  exact Finset.single_le_sum (f := id) (fun _ _ => Nat.zero_le _) h_mem
